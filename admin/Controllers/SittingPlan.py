...
def uploadHall(request):
    if request.method == 'POST':
        ...
        status = Hall.objects.create(
            title=title, location=location, available_seats=capacity)
        if status:
            messages.success(request, 'Hall Information added successfully')
        else:
            messages.error(
                request, 'Something goes wrong please try again....')
...
def uploadSlot(request):
    if request.method == 'POST':
        ...
        status = Slot.objects.create(
            start_time=start_time, end_time=end_time, seat_limits=limit)
        if status:
